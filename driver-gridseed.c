...
static const char *gridseed_version = "v3.7.2.20140409.02";
...
typedef struct s_gridseed_info {
	...
	// KHS meter
	struct timeval	workstart_time;
	struct timeval	last_nonce_time;
	int		hashrate;
	// options
	int		baud;
	int		freq;
...
static bool gridseed_detect_one(libusb_device *dev, struct usb_find_devices *found)
{
	...
	cgtime(&(info->last_nonce_time));

	get_options(info, opt_gridseed_options);
	info->hashrate = (int)(info->chips * info->freq / 11.0);
...
static bool gridseed_send_task(struct cgpu_info *gridseed, struct work *work)
{
	...
	ret = (gc3355_write_data(gridseed, cmd, sizeof(cmd)) == 0);
	if (ret)
		cgtime(&info->workstart_time);
	return ret;
}
...
static struct api_data *gridseed_api_stats(struct cgpu_info *gridseed)
{
	...
	root = api_add_int(root, "DevKHs", &(info->hashrate), false);
	root = api_add_timeval(root, "LastNonceAt", &(info->last_nonce_time), false);
	return root;
}

static void gridseed_get_statline(char *buf, size_t siz, struct cgpu_info *gridseed) {
	GRIDSEED_INFO *info = gridseed->device_data;
	tailsprintf(buf, siz, " R:%dKh/s", info->hashrate);
	...
	while (!thr->work_restart && (ret = gc3355_get_data(gridseed, buf, GRIDSEED_READ_SIZE)) == 0) {
		...
		if (buf[0] == 0x55 || buf[1] == 0x20) {
			...
			if (!submit_nonce(thr, work, nonce))
				...
				uint32_t hashs;
				cgtime(&(info->last_nonce_time));
				elapsed_ms = ms_tdiff(&now, &info->workstart_time);
				hashs = nonce - chip * ((uint32_t)0xffffffff / info->chips);
				info->hashrate = (int)((double)hashs / elapsed_ms * info->chips);
				applog(LOG_DEBUG, "%d 0x%08x HASHS: %d, %dms, %dKh/s", chip, nonce, hashs,
						elapsed_ms, info->hashrate);
			}
		} else {
